...
var _hasEventListeners = !!window.addEventListener,
  ...
  _onClickMenu = function (evt) {
    ...
    if (target.parentElement.children.length == 1) {
      ...
      for (var i = 0; i < elm.length; i++) elm[i].classList.remove("current");
      // adds 'current' class
      ...
      document
        .getElementById(target.href.replace(/.*(?=#)#/, ""))
        .classList.add("show");
  ...
  addCurrentItemClass = function (elm) {
    ...
    if (prn.className != "clearfix") addCurrentItemClass(prn.parentElement);
  };
